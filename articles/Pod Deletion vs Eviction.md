...
2. `Runing` 的 Pod:
- 若处于 Termianting (DeletionTimestamp != nil)，执行 4
...
4. 为 Pod 添加 Evcition 的 Condition 后直接执行删除
...
> A: 大体来说，Evcition 是在有预期的受控删除时，一些典型的场景：
...
> A：因为 Eviction 只是 Pod 对象下的一个子资源，相似的还有 Binding 对象等。

* Q: 有一个控制器来负责 eviction 吗？
...
* Q: k8s 内部都有哪些场景会调用 Evict？
> A: 看下来就 drain node 的时候调用了 Eviction。

* Q: NodeNotReady 之后，发生的该 Node 上所有的 Pod 都被删除的场景，是通过驱逐实现的吗？
> A: 不是，是通过 Deletion API 实现的。为什么要这么做？我理解是因为此种情况下，kubelet 已经不工作或者断连了，所以在该 Node 上的 Pod 的状态也是不可靠的（Unkonwn），因此执行 Eviction 来尽可能的保护这部分 Pod 已经没有意义，因此直接执行了 Deletion。\
